...
import type { ContextMenuItem } from "@termweave/contracts";
import { layer as NetServiceLayer, NetService } from "@termweave/shared/Net";
import { RotatingFileSink } from "@termweave/shared/logging";
...
const DESKTOP_SCHEME = "termweave";
const ROOT_DIR = Path.resolve(__dirname, "../../..");
const isDevelopment = Boolean(process.env.VITE_DEV_SERVER_URL);
const APP_DISPLAY_NAME = isDevelopment ? "Termweave (Dev)" : "Termweave";
const APP_USER_MODEL_ID = "com.termweave.app";
const USER_DATA_DIR_NAME = isDevelopment ? "termweave-dev" : "termweave";
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { termweaveCommitHash?: unknown };
    return normalizeCommitHash(parsed.termweaveCommitHash);
  } catch {
    return null;
  }
}
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Termweave failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Termweave ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/Termweave (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`termweave`). If the legacy
